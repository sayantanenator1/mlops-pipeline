...
def main(args):
    logging.info("Reading X and y")
    logging.info(f"X on GCS at {args.x_path}, starting download")
    X = get_gcs(args.x_path)
    logging.info(f"y on GCS at {args.y_path}, starting download")
    y = get_gcs(args.y_path)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20)
